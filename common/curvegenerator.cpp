...
void CurveGenerator::generate()
{
    ...
    const uint32_t pts_per_line = 4;
    const uint32_t pt_count = ptls_size * pts_per_line;
    const uint32_t verts_per_strip = 18;
    ...
    for (uint32_t idx =0; idx<ptls_size; ++idx){
        ...
        PrevPoint[pt_idx+0] = glm::vec4(point_list[line_idx-1], +(50.1f+radiusList[idx]));
        NextPoint[pt_idx+0] = glm::vec4(point_list[line_idx+1], +(50.1f+radiusList[idx]));
        ...
        Positions[pt_idx+3] = point_list[idx];
        PrevPoint[pt_idx+3] = glm::vec4(point_list[line_idx-1], -(50.1f+radiusList[idx]));
        NextPoint[pt_idx+3] = glm::vec4(point_list[line_idx+1], -(50.1f+radiusList[idx]));
        ...
        Indices[vert_idx+ 2] = 5 + pt_idx;

        Indices[vert_idx+ 3] = 0 + pt_idx;
        Indices[vert_idx+ 4] = 5 + pt_idx;
        Indices[vert_idx+ 5] = 4 + pt_idx;

        // middle
        Indices[vert_idx+ 6] = 1 + pt_idx;
        Indices[vert_idx+ 7] = 2 + pt_idx;
        Indices[vert_idx+ 8] = 6 + pt_idx;

        Indices[vert_idx+ 9] = 1 + pt_idx;
        Indices[vert_idx+10] = 6 + pt_idx;
        Indices[vert_idx+11] = 5 + pt_idx;

        // right edge
        Indices[vert_idx+12] = 2 + pt_idx;
        Indices[vert_idx+13] = 3 + pt_idx;
        Indices[vert_idx+14] = 7 + pt_idx;

        Indices[vert_idx+15] = 2 + pt_idx;
        Indices[vert_idx+16] = 7 + pt_idx;
        Indices[vert_idx+17] = 6 + pt_idx;
    }
}
